...
//Application process for job seeker
...
//Delete application process for job seeker
...
export const postApplication = catchAsyncError(async (req, res, next) => {
  ...
  if (!req.files || Object.keys(req.files).length === 0) {
    return next(new ErrorHandler("Please upload a Resume"));
  }
  ...
  if (!allowedFormats.includes(resume.mimetype)) {
    return next(
      new ErrorHandler(
        "Invalid file type, Please upload a valid Resume in a PNG, JPG or WEBP format",
        400
      )
    );
  }

  // handles file uploads using Cloudinary and processes the form data to create a new job application
  ...
  if (!cloudinaryResponse || cloudinaryResponse.error) {
    console.log(
      "Cloudynary Error:",
      cloudinaryResponse.error || "Unknown cloudanary error"
    );
